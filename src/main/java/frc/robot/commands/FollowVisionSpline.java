...
public class FollowVisionSpline extends CommandBase {
    ...
    public static final double startSpeedDecreaseDistance = 2.0; // number of meters away from portal the robot starts
                                                                 // slowing down
    ...
    protected void execute() {
        position.updatePosition(PIDInfo.getInstance().getLeftEncoderDelta(),
                PIDInfo.getInstance().getRightEncoderDelta());
    }
    ...
    // distance of the robot from the spline. Second, the difference between the
    // angle
    // of the spline and the angle of the robot. Third, the angle change of the
    // spline.
    ...
    public double getWantedSpeed() {
        double distanceFromEnd = Math
                .sqrt(Math.pow(path.endpoint[0] - position.x, 2) + Math.pow(path.endpoint[1] - position.y, 2));

        double returnVal = distanceFromEnd * distanceFromEnd
                / (startSpeedDecreaseDistance * startSpeedDecreaseDistance);
