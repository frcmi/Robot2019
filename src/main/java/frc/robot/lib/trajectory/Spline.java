...
public class Spline {
    ...
    public static boolean reticulateSplines(double x0, double y0, double theta0, double x1, double y1, double theta1,
        ...
        double theta1_hat = SnailMath.getDifferenceInAngleRadians(result.theta_offset_, theta1);
        ...
        if (Math.abs(SnailMath.getDifferenceInAngleRadians(theta0_hat, theta1_hat)) >= Math.PI / 2) {
            return false;
        }
    ...
    public double angleAt(double percentage) {
        double angle = SnailMath.boundAngle0to2PiRadians(Math.atan(derivativeAt(percentage)) + theta_offset_);
        return angle;
    }

    public double angleChangeAt(double percentage) {
        return SnailMath.boundAngleNegPiToPiRadians(Math.atan(secondDerivativeAt(percentage)));
    }
